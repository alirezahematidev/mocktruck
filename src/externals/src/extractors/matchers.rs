...
impl Char {
    ...
    pub fn new(length: Option<u16>, style: Option<String>) -> Char {
        Char { length, style }
    }
...
impl Image {
    #[wasm_bindgen(constructor)]
    pub fn new(width: Option<u16>, height: Option<u16>) -> Image {
        Image { width, height }
    }
...
#[wasm_bindgen]
pub fn number_arguments_extract(text: &str) -> Option<u16> {
    lazy_static! {
        static ref RE: Regex = Regex::new(r"(?i)^number\(\s*(?P<length>\d+)\s*\)$").unwrap();
    }
...
#[wasm_bindgen]
pub fn char_arguments_extract(text: &str) -> Option<Char> {
    lazy_static! {
        static ref RE: Regex = Regex::new(
            r"(?i)^char\(\s*(?P<length>\d+)?\s*,?\s*(?P<style>(upper|lower)case)?\s*\)$"
        )
        .unwrap();
    }
